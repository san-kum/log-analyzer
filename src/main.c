...
main (int argc, char **argv)
{
  ...
  if (!success)
    {
      ...
      for (int i = 0; i < entry_count; i++)
        log_parser_free_entry (entries[i]);
      free (entries);
      log_analyzer_cleanup (ctx);
      return EXIT_FAILURE;
    }
  ...
  if (!success)
    fprintf (stderr, "Failed to write summary report\n");

  success = report_generator_write_detailed (ctx);
  if (!success)
    fprintf (stderr, "Failed to write detailed report\n");
...
{
  printf ("Log Analyzer version: %s\n", VERSION);
}

void
cli_print_help (void)
{
  printf ("Log Analyzer - A tool for analyzing logs and recommending "
          "performance improvements\n\n");
  printf ("Usage: log_analyzer [OPTIONS] INPUT_FILE\n\n");
  printf ("Options:\n");
  printf ("  -o, --output FILE     Write output to FILE (default: stdout)\n");
  printf ("  -f, --format FORMAT   Specify the log format (default: "
          "auto-detect)\n");
  ...
  printf (
      "  log_analyzer -o recommendations.txt -f syslog /var/log/kern.log\n");
}
